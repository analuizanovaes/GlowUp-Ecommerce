...
// Vetores que atuam como o banco de dados temporário da aplicação
// ============================================================
...
// DESCRIÇÃO: Popula o catálogo com produtos padrão ao abrir o app.
// ============================================================
...
// Interfaces Gráficas via Terminal
...
int main(void) {
    ...
    int usuario_logado = 0;   // 0 = Falso (Deslogado), 1 = Verdadeiro (Logado)
    int indice_cliente = -1;  // Guarda a posição do usuário no vetor "usuarios[]"

    exibirBoasVindas();   
    inicializarSistema(); 

    // Laço principal de execução do sistema
    do {
        // Exibe o menu adequado ao estado de autenticação atual
        if (!usuario_logado) {
            exibirMenuDeslogado();
        } else {
            exibirMenuLogado();
        ...
        // Validação contra letras digitadas acidentalmente pelo usuário
        if (scanf("%d", &opcao) != 1) {
        ...
        if (!usuario_logado) {
            ...
            case 2: 
                ...
                if (indice_cliente != -1) {
                    usuario_logado = 1; // Altera o estado da sessão
        ...
        // ====================================================
            ...
            case 3: { 
                ...
                if (acao_carrinho == 1) {
                    ...
                    adicionarProdutoCarrinho(id, qtd); // Função modificada com as novas validações
                }
                break;
            }
            ...
            case 5: 
                // Acesso ao pós-venda passando o índice do cliente para filtro de dados
                menu_pos_venda(indice_cliente); 
                break;
            ...
            case 0: 
                // Operação de Logout
                usuario_logado = 0;
                indice_cliente = -1; // Remove a referência ao usuário anterior
    ...
    // O laço mantém a execução se a opção for diferente de zero OU se o usuário estiver logado
