...
{
    if (strstr(endereco_digitado, "Brasil") != NULL || strstr(endereco_digitado, "brasil") != NULL || strstr(endereco_digitado, "BRASIL") != NULL)
    {
        return 1;
    }
    else
...
int processarPagamento(float total_pagar)
{
    ...
    if (scanf("%d", &opcao) != 1)
    {
        ...
        while (getchar() != '\n')
            ;
        return 0; // O retorno 0 cancela o pagamento
    }
    while (getchar() != '\n');
...
void faturarPedido(float total_final, int indice_cliente)
{
    ...
    for (int i = 0; i < qtd_itens_carrinho; i++)
    {
        for (int j = 0; j < total_produtos; j++)
        {
            ...
            {
                if (catalogo[j].estoque >= carrinho[i].quantidade)
                {
