...
O projeto está sendo construído inteiramente na linguagem **C**, aplicando os conceitos de Programação Estruturada. 

* **Escopo Acadêmico:** Como este é um projeto focado exclusivamente na disciplina de Programação Estruturada, ele **não** utiliza Programação Orientada a Objetos (POO), frameworks complexos ou bancos de dados reais. Todo o design segue estritamente o paradigma estruturado da linguagem C.
* **Armazenamento:** Os dados de usuários, produtos e estoque são manipulados de forma volátil (em memória) através de Structs e Vetores Globais.
* **Modularização:** O código é organizado em funções para manter a clareza e facilitar a manutenção e os testes.
...
* **Controle Financeiro e Logístico:** Simulação de Gateway de Pagamento e faturamento de compras (HG01 e HG02).
